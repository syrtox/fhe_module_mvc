...
(function ($, window, document) {
    ...
    $.JS_BRAMUS.JsProgressBar = function (elmt, percentage, options) {
        ...
        base.$el = $(elmt);                     // Element where to render the progressBar in
        base.id = base.$el.attr('id');          // Unique ID of the progressbar
        ...
        base.init = function () {
            ...
            // get the options
            base.options = $.extend({}, $.JS_BRAMUS.JsProgressBar.defaultOptions, options);
            ...
            if (!$.isArray(base.options.barImage)) {
        ...
        var _setBgPosition = function (percentage) {
            ...
            $("#" + base.id + "_percentImage").css('backgroundPosition', (base.initialPos + (percentage * base.pxPerPercent)) + "px 50%");
            ...
            if ((newBackIndex != base.backIndex) && (base.options.barImage[newBackIndex] != undefined)) {
                $("#" + base.id + "_percentImage").css('backgroundImage', "url(" + base.options.barImage[newBackIndex] + ")");
            }

            base.backIndex = newBackIndex;

            // Adjust the alt & title of the image
            $("#" + base.id + "_percentImage").attr('alt', percentage + "%");
            $("#" + base.id + "_percentImage").attr('title', percentage + "%");

            // Update the text
            if (base.options.showText == true) {
                $("#" + base.id + "_percentText").html("" + percentage + "%");
            }
    ...
    // Default Options
    $.JS_BRAMUS.JsProgressBar.defaultOptions = {
        autoHook:true,
        ...
        barImage	: Array(
						...
						), // Image to use in the progressbar. Can be an array of images too.
